"""Multi-seed launcher preset for Alibaba r432 (r431-base defensive continuation).

Wrapper around `altgan.launch_chunk_surface_multiseed` that pins:
...
- r431 banked bases + donor bank
- r431 accepted move JSONs as `--priority-moves` (passed as globs so the shell
...
def main() -> int:
    ...
    defaults = [
        ...
        ",".join(
            [
                f"{eval_root}/alibaba_chunksurf_r431_r426base4_cap16_ck4_seed*_moves.json",
        ...
        "--base-template",
        f"{output_root}/alibaba_chunksurf_r431_r426base4_cap16_ck4_seed{{seed}}_fake_1000k.csv",
        "--donor-globs",
        ",".join(
            [
                ...
                f"{output_root}/alibaba_chunksurf_r43*_seed{{seed}}_fake_1000k.csv",
