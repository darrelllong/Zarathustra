...
- **alibaba_v201** — **LAUNCHED** (2026-04-21). **IDEA #54: Gumbel-Softmax hard binary reuse head**. Seed=5. Hot-start: v195 `pretrain_complete.pt`. Config: hidden=256, latent=24, noise=10, n_critic=2, files=8, records=15k, wgan-sn, no-amp, no-compile. `--gumbel-reuse --gumbel-reuse-weight 1.0 --gumbel-tau-start 1.0 --gumbel-tau-end 0.5`. Gumbel-Softmax replaces scalar reuse output with hard 0/1 (straight-through), supervised by BCE-logit (class-balanced pos_weight up to 20×). Hard binary decision fed to WGAN critic. Rate emerges naturally from cross-entropy calibration (no explicit target). **Acceptance bar**: ep5 EMA recall > 0.5 AND reuse_rate ∈ [0.15, 0.35] (approaching 0.265 target). PID=3517685 on vinge.local. Log `/home/darrell/train_alibaba_v201.log`.
