...
- **alibaba_v190** — **IDEA #36 seed=3 reproducibility test**. Same recipe as v189 (boundary-critic-weight 0.5, k=4, n-regimes 4, seed=3). Pretrain_complete.pt copied from v189 (= v176 pretrain). Launched 2026-04-20 18:06. **bc_gap positive from ep1** (0.31→0.23→0.15→0.14 ep1-4) — boundary critic IS discriminating real vs fake boundaries, not immediately disabled. Seed bundle for IDEA #36: v189 seed=7 (running), v190 seed=3 (this run). Log `/home/darrell/train_alibaba_v190.log`. PID 2649277.
- **alibaba_v189** — **IDEA #36 (Learned Boundary Prior)** on Alibaba. v176 pretrain reused (seed=7, phases 1–2 skipped); Phase 3 uses fresh LSTM G with `BoundaryCritic` WGAN-SN MLP (K=4, hidden=128, weight=0.5). All hand-written BS/OC scalars removed (weight=0). PCF and multi-scale-critic also dropped (per Round 33 / Round 32 guidance: drop those for Alibaba). `--n-regimes 4` (not 8 — 4 regimes are Alibaba-optimal per v46). Seed=7 to match v176 pretrain. First structural Alibaba run targeting boundary continuity without the palindrome bias. **ep5: ★=0.071**; **ep10: ★=0.082**; **ep15: ★=0.073**; **ep20: ★=0.099**; **ep30: ★=0.054 (train-best, recall=0.757)**; **ep35: ★=0.103, recall=0.586** (noisy oscillation — recall 0.58→0.76→0.59 pattern; ep30 train-best stands). Note: v189 does not log bc_gap (launched before that instrumentation); v190 (same recipe, seed=3) shows bc_gap=0.31 at ep1 confirming boundary critic is active and discriminating. Log `/home/darrell/train_alibaba_v189.log`. PID 2137428.
