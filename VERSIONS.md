...
- **alibaba_v198** — **LAUNCHED** (2026-04-21). **IDEA #48: post-hoc LRU stack decoder**. Architecture: new module `llgan/lru_stack_decoder.py` — replaces generated obj_id stream with explicit LRU stack output, conditioned on generator's obj_id_reuse signal + corpus-fitted stack-distance bucket PMF. First experiment: apply decoder post-hoc to v195 ep110 (frozen-best ★=0.04204) and measure HRC-MAE before/after. Target: HRC-MAE < 0.01 alibaba (beat NeuralAtlas 0.00183 is stretch; prove improvement over v195 HRC-MAE=0.1287 is minimum bar). CLI: `python generate.py --checkpoint epoch_0110.pt --lru-stack-decoder --lru-stack-corpus alibaba --lru-stack-real-csv <real_trace.csv>`. Phase 2 (if phase 1 succeeds): integrate LRU stack into training as a head with bucket NLL supervision. Log: TBD on vinge.local.
