...
- **alibaba_v197** — **RUNNING** (launched 2026-04-21 17:05). **IDEA #45: chained-window ACF loss on obj_id_reuse**. Seed=5. Hot-start: v195 `pretrain_complete.pt` (E/R weights loaded; G LSTM reinitialised — pretrain_complete.pt had SSM params). v195 recipe minus `--boundary-critic-*` (bc branch closed) plus `--acf-chain-weight 1.0 --acf-chain-windows 4 --acf-chain-n-lags 10`. Acceptance bar: frozen ★ ≤ 0.042 (match v195 ATB) AND long-rollout reuse_access > v176's 0.046. If reuse_access does not improve, architecture slot → IDEA #47. Log `/home/darrell/train_alibaba_v197.log`.
