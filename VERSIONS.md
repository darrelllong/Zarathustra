...
- **alibaba_v195** — **RUNNING** (launched 2026-04-21). **IDEA #44 domain-matched decoded bc** (`--boundary-critic-real-reconstruct`). Seed=5. Pretrain: v193 `pretrain_complete.pt` (seed=5). Same recipe as v194 except real bc positives are `R(E(real))` instead of raw normalized arrays — both real and fake in the same decoded feature space. Removes raw-vs-decoded shortcut confirmed in Round 37 P1 #1. EMA so far: ep20 recall=0.683, comb=0.075; ep25 recall=0.711, comb=0.076; ep30 recall=0.623, comb=0.095 (mild dip). W=0.71-1.13 (stable, no instability). G-loss +0.15-0.38 (very balanced, unlike v194 post-collapse G=+7). Log `/home/darrell/train_alibaba_v195.log`.
