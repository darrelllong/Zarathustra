...
- **alibaba_v202** — **LAUNCHED** (2026-04-21). **IDEA #54 v2: direct-from-hidden reuse_head (nn.Linear bypass)**. Seed=5. Hot-start: v195 `pretrain_complete.pt`. Config: hidden=256, latent=24, noise=10, n_critic=2, files=8, records=15k, wgan-sn, no-amp, no-compile. `--gumbel-reuse --gumbel-reuse-weight 1.0 --gumbel-tau-start 1.0 --gumbel-tau-end 0.5`. New: `nn.Linear(hidden_size, 1)` head on LSTM output `h` (before Recovery decoder). Gradient path: BCE-logit → logits → h_t → G (no R bottleneck). Removes the Recovery Jacobian bottleneck that killed v201. BCE-logit class-balanced (pos_weight = n_neg/n_pos, capped at 20×). Hard Gumbel-Softmax decision replaces `fake_decoded[:,obj_id_col]` for WGAN input. **Acceptance bar**: reuse_rate ∈ [0.15, 0.30] by ep10; frozen ★ ≤ 0.050; long-rollout HRC-MAE < 0.010 without oracle override; mark quality panel after denormalized CSV generation. PID=3523094 on vinge.local. Log `/home/darrell/train_alibaba_v202.log`.
