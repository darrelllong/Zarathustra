...
- **alibaba_v204** — **RUNNING** (2026-04-22, ep60+). **IDEA #54 v4: soft-sigmoid reuse_head, BCE weight=3.0**. Seed=5. Config identical to v203 except `--gumbel-reuse-weight 3.0`. PID=3575193 on vinge.local. **GAN trajectory**: reuse_rate oscillates 0.01-0.20 (peaks at ep12=0.165, ep26=0.202, ep50=0.150, ep60=0.148); target 0.265 NOT reached. However, EMA★ is the best trajectory ever recorded: ep10=0.099, ep40=0.081 ★, ep55=0.077 ★, ep60=**0.057** ★ (recall=0.770). The strong recall improvement (0.494→0.660→0.770) despite oscillating reuse suggests the weight=3.0 BCE is injecting enough gradient to improve overall feature learning. W distance grows from 0.83→3-6 over ep1-60 (critic strengthening). **Decision**: let run to ep100-150 and frozen-sweep — ★ trajectory may reach < 0.042 (new ATB). Reuse fix pending IDEA #57 (gradient-stop on reuse column from WGAN). Log `/home/darrell/train_alibaba_v204.log`.
