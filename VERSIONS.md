...
- **alibaba_v205** — **LAUNCHED** (2026-04-22). **IDEA #57: gradient-stop on h for BCE reuse**. Seed=5. Config: same as v204 but `--gumbel-reuse-weight 1.0` (reduced from 3.0 since BCE now has clean gradient path). Key fix: `G.reuse_head(G._last_h_for_reuse.detach())` in train.py. h is detached before passing to reuse_head for BCE — WGAN trains all G params through H_fake (including LSTM), BCE trains ONLY reuse_head.weight (not LSTM), breaking the WGAN/BCE competition on h. Verified: LSTM grad norm from BCE = 0.000000, reuse_head grad norm from BCE = 0.979. PID=3604298 on vinge.local. **Acceptance bar**: reuse_rate ∈ [0.20, 0.30] and stable (not oscillating) by ep10; frozen ★ ≤ 0.050; long-rollout HRC-MAE < 0.010. Log `/home/darrell/train_alibaba_v205.log`.
