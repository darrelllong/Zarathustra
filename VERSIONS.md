...
- **alibaba_v199** — **LAUNCHED** (2026-04-21). **IDEA #51: global reuse-rate matching loss** to fix dead reuse signal. Seed=5. Hot-start: v195 `pretrain_complete.pt`. Config: hidden=256, latent=24, noise=10, n_critic=2, files=8, records=15k, loss=wgan-sn, no-amp, no-compile. `--reuse-rate-loss-weight 10.0 --reuse-rate-target 0.265`. Loss: `L_rate = λ × (mean(sigmoid(reuse_raw)) - 0.265)²` appended to G loss inside copy-path block. Per-epoch log: `reuse_rate=X.XXXX`. AMP BCE autocast bug fixed (float32 cast on fake_p/real_p/weight). **Acceptance bar**: training reuse_rate ∈ [0.24, 0.29] at convergence AND frozen ★ ≤ 0.050 (must not sacrifice short-window quality). Long-rollout sidecar target at ep30: HRC-MAE < 0.015 with LRU decoder. PID=3499695 on vinge.local. Log `/home/darrell/train_alibaba_v199.log`.
