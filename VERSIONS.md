...
- **alibaba_v203** — **LAUNCHED** (2026-04-21). **IDEA #54 v3: soft-sigmoid reuse_head (no Gumbel)**. Seed=5. Config: hidden=256, latent=24, noise=10, n_critic=2, files=8, records=15k, wgan-sn, no-amp, no-compile. `--gumbel-reuse --gumbel-reuse-weight 1.0`. Key fix vs v202: **hard Gumbel-STE replaced with soft sigmoid**. The critic sees `sigmoid(logit)*2-1` (continuous ∈[-1,1]) instead of a hard {-1,+1} decision. BCE-logit trains the head; WGAN + BCE jointly converge to equilibrium rate. Gradient path unchanged: BCE→logits→h→G. PID=3538906 on vinge.local. **Acceptance bar**: reuse_rate ∈ [0.15, 0.30] by ep10; frozen ★ ≤ 0.050; long-rollout HRC-MAE < 0.010. Log `/home/darrell/train_alibaba_v203.log`.
