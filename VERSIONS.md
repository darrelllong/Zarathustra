...
- **alibaba_v190** — **IDEA #36 seed=3 reproducibility test**. Same recipe as v189 (boundary-critic-weight 0.5, k=4, n-regimes 4, seed=3). Pretrain_complete.pt copied from v189 (= v176 pretrain). Launched 2026-04-20 18:06. **bc_gap positive ep1-9** (0.31→0.09→0.11 ep1-9, stabilizing ~0.10) — boundary critic discriminating throughout. **ep5: MMD²=0.038, recall=0.613, ★=0.115 ← AVOIDS COLLAPSE** (recall=0.613 >> 0.26 collapse threshold; confirms IDEA #36 avoids Alibaba collapse at seed=3, not just seed=7). Seed bundle: v189 seed=7 ★=0.054 ep30, v190 seed=3 ★=0.115 ep5 — both healthy. Log `/home/darrell/train_alibaba_v190.log`. PID 2649277.
- **alibaba_v189** — **IDEA #36 (Learned Boundary Prior)** on Alibaba. v176 pretrain reused (seed=7, phases 1–2 skipped); Phase 3 uses fresh LSTM G with `BoundaryCritic` WGAN-SN MLP (K=4, hidden=128, weight=0.5). All hand-written BS/OC scalars removed (weight=0). PCF and multi-scale-critic also dropped (per Round 33 / Round 32 guidance: drop those for Alibaba). `--n-regimes 4` (not 8 — 4 regimes are Alibaba-optimal per v46). Seed=7 to match v176 pretrain. First structural Alibaba run targeting boundary continuity without the palindrome bias. **ep5: ★=0.071**; **ep10: ★=0.082**; **ep15: ★=0.073**; **ep20: ★=0.099**; **ep30: ★=0.054 (train-best, recall=0.757)**; **ep35: ★=0.103, recall=0.586**; **ep40: ★=0.093, recall=0.591** (recall oscillation: peaks at ep5/ep15/ep30 improving 0.709→0.733→0.757; valleys 0.577→0.586→0.591 also recovering). ep30 train-best stands. Note: v189 predates bc_gap instrumentation; v190 bc_gap confirms boundary critic active (~0.10 sustained). Log `/home/darrell/train_alibaba_v189.log`. PID 2137428.
