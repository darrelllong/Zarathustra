...
- **tencent_v187** — **KILLED ep80 (2026-04-20 17:44); frozen sweep running**. v165 EXACT −`--multi-scale-critic` + seed=5. Full trajectory: ep5 ★=0.128, ep25 ★=0.121, ep35 ★=0.089, **ep50 ★=0.087 (train-best)**, ep70 ★=0.109, ep80 ★=0.110 (stale, no improvement past ep50). Frozen sweep `/home/darrell/tencent_v187_frozen_sweep.log` running — result pending.
- **tencent_v188** — v165 EXACT recipe **minus** `--mixed-type-recovery` + `--seed 5`. **Mixed-type Recovery ablation** — 4th and final tencent component ablation, completes the audit matrix. Keeps retrieval-memory + multi-scale-critic + PCF-loss 2.0 + 8 regimes. Log `/home/darrell/train_tencent_v188.log`. PID 1046763. **Trajectory (2026-04-20)**: ep5 ★=0.168, ep20 ★=0.109, ep40 ★=0.095 (train-best), ep55 ★=0.111, ep65 ★=0.112 (regressing). Currently ep65; **kill threshold ep70** (~5 more epochs, ~26 min).
- **alibaba_v189** — **IDEA #36 (Learned Boundary Prior)** on Alibaba. v176 pretrain reused (seed=7, phases 1–2 skipped); Phase 3 uses fresh LSTM G with `BoundaryCritic` WGAN-SN MLP (K=4, hidden=128, weight=0.5). All hand-written BS/OC scalars removed (weight=0). PCF and multi-scale-critic also dropped (per Round 33 / Round 32 guidance: drop those for Alibaba). `--n-regimes 4` (not 8 — 4 regimes are Alibaba-optimal per v46). Seed=7 to match v176 pretrain. First structural Alibaba run targeting boundary continuity without the palindrome bias. **ep5 (2026-04-20): EMA MMD²=0.01282, β-recall=0.709, ★=0.07102 (train-best) ← HEALTHY BASIN**; **ep10: MMD²=0.02007, recall=0.691, ★=0.082**; **ep15: MMD²=0.01976, recall=0.733, ★=0.073**; **ep20: MMD²=0.01420, recall=0.577, ★=0.099** (recall noisy 0.58–0.73 through ep20; MMD² improving 0.020→0.014; ep5 still train-best; run healthy above collapse floor). Log `/home/darrell/train_alibaba_v189.log`. PID 2137428.
