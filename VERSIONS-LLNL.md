...
### tencent_v235 — CLOSED-INCONCLUSIVE (2026-04-30 01:33) — IDEA #117 + IDEA #116 on v234d pretrain ≈ baseline

**Recipe:** v229 base recipe + `--retrieval-train-carry` (NEW IDEA #117) + `--long-chain-weight 2.0 --long-chain-windows 10` (IDEA #116). Pretrain reused from v234d (NOT v229 — v234d's was the latest fresh pretrain under current code). seed=5, w-stop 7.0, no `--lru-cache-depth` (legacy preprocessor matching v234d's pretrain).

**First-ever working LRU diagnostic** (commit `9e0c001` sibling-import fix landed before launch). ep5 / ep10 readouts:

| | ep5 | ep10 |
|---|---|---|
| W | 2.24 | 2.41 |
| reuse_rate (train, per-window) | 0.0248 | 0.0451 |
| EMA MMD² | 0.02428 | 0.01080 |
| recall (train EMA) | 0.546 | 0.472 |
| train ★ | 0.115 | **0.116** |
| **lru_actual** (carried-state, IDEA #115) | **0.990** | **0.993** |
| **lru_fp** (carried-state unique objs) | **51** | **33** |

**Frozen-bundle gate at ep10:**

| | v235 ep10 | v234d ep10 | delta |
|---|---|---|---|
| MMD² | 0.01214 | 0.01539 | -21% (better) |
| β-recall | 0.075 | 0.091 | -18% (worse) |
| α-precision | 0.842 | 0.798 | +6% (better) |
| **frozen ★** | **0.19714** | **0.19719** | **-0.00005 (~0%)** |

**Verdict — IDEA #117 + IDEA #116 closed-INCONCLUSIVE on v234d pretrain.** Frozen ★ is bit-equivalent to v234d's baseline (0.197). The new losses produce slightly tighter MMD² and precision but worse recall — net: **the same operating point as v234d, on this pretrain**. The carried-state diagnostic actually got WORSE under training: lru_fp 51→33 across ep5→ep10 (model converging to fewer unique objects in long rollout, the opposite of what IDEA #117 was supposed to do).

LANL R10's methodology concern is now empirically validated: building IDEA #117 on top of v234d's contaminated pretrain cannot distinguish "IDEA #117 doesn't work" from "v234d pretrain pre-determined the trajectory." The clean test would be fresh pretrain → IDEA #117 from scratch, costing 3.5h. **Given the lru_fp=51→33 deterioration trend, that fresh-pretrain rerun is unlikely to produce a frozen ★ < 0.10**, but a clean negative result has more diagnostic value than this inconclusive one.

**What IS confirmed**:
1. The IDEA #117 implementation (`9d89806`) is functionally correct — `--retrieval-train-carry` flag works, retrieval_state threads through long-chain windows without crashing.
2. The LRU diagnostic fix (`9e0c001`) works — first-ever readouts of `lru_actual` / `lru_fp` mid-training.
3. **The carried-state mode collapse hypothesis is empirically established.** v235 ep5 lru_actual=0.990 / lru_fp=51 → ep10 lru_actual=0.993 / lru_fp=33 means the carried-state Markov dynamics converge to a tiny (~30 object) attractor regardless of training pressure. This is the v229 ep20 collapse mechanism Gemini Round 3 P1 #2 / IDEA #117 hypothesised, now visible in real-time.
