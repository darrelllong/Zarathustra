...
r422 completed as another non-banked learned-generator scout. It is slightly
better than r420 and much better than the unconstrained learned-birth rollout,
but still far behind the banked Tencent r336 result. The official panel shows
the same structural miss across seeds: LRU/FIFO/SLRU fake HRC is too low at
32/128, while SIEVE is often too high, so the next ML move should bias the
learned Mattson decoder toward more immediate reuse rather than only controlling
birth count.
...
vinge as a no-fit rollout from the r437 checkpoint. This keeps the r434/r437
token geometry and changes only the learned Denning WS decoding path:
impossible per-window WS bins are masked and renormalized before target counts
feed birth/reuse control.
...
## 2026-05-09 11:29Z -- Tencent r440 Max-Window WS-Edge Fit Launched

Launched `tencent_mdlstm_r440_wsmax_empiricalrank_norecycle_ws_p3` on vinge as
an actual fit. This is the training-time architecture scout from r439: Mattson
rank tokens stay at r434 geometry, empirical within-bin rank sampling stays on,
but Denning WS tokens are binned against the largest configured WS window
instead of the full trace footprint.

Process line confirms the intended fit:

`--ws-edge-mode max-window --recycle-rank-cap 0 --rank-sampler empirical --exact-rank-cutoff 0 --seeds 42,80,81,82 --temperature 1.0 --short-reuse-pressure 3.0 --fit --birth-control-mode ws`

Tokenization confirms the new WS target geometry:

`[mattson_denning tokenize] n=100,000 footprint=38,507 rank_vocab=59 reuse_offset=1 recycle_rank_cap=0 exact_rank_cutoff=0 fresh=38,507 recycle=0 reuse=61,493 ws_bins=30 ws_edge_mode=max-window ws_edge_max=8192 windows=[32, 128, 512, 2048, 8192]`

PID: `4076854`.
Log:
`/tiamat/zarathustra/altgan-output/logs/tencent_mdlstm_r440_wsmax_empiricalrank_norecycle_ws_p3_vinge_20260509T112948Z.log`.
Model:
...
## 2026-05-09 12:14Z -- Tencent r442 Phase-Conditioned Fit Launched

Launched `tencent_mdlstm_r442_pos32_wsmax_empiricalrank_norecycle_ws_p3` on
vinge as a real fit. This uses the r440 max-window Denning geometry, empirical
within-bin rank sampling, no recycle cap, and the new learned phase embedding
path.
