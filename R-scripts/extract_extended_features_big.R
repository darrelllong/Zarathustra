#!/usr/bin/env Rscript
# Variant of extract_extended_features.R for the largest traces:
#   * per-series time budget = 1800s (was 90s in the slim version)
#   * Lomb sample = 20000 (was 5000)
#   * spec/cepstrum/perm_entropy at 100K cap
#   * ApEn/SampEn at 2000 cap
# Same metric set as the slim version (no HMM, no DFA, no change-points
# — those are the wall-time killers and we accept those gaps as before).
#
# Usage: extract_extended_features_big.R MANIFEST.csv OUT_DIR [MAX_RECORDS]
...
manifest_path <- if (length(args) >= 1) args[[1]] else stop("usage: MANIFEST OUT_DIR [MAX_RECORDS]")
out_dir <- if (length(args) >= 2) args[[2]] else stop("missing OUT_DIR")
max_records <- if (length(args) >= 3) as.integer(args[[3]]) else 1000000L
dump_py <- file.path(Sys.getenv("HOME"), "LLNL", "Zarathustra", "R-scripts",
                     "dump_trace_records.py")
if (!file.exists(dump_py)) {
  alt <- "/tiamat/zarathustra/r-analysis-src/R-scripts/dump_trace_records.py"
  if (file.exists(alt)) dump_py <- alt
}
...
cat(sprintf("[big] manifest=%s out=%s max_records=%d\n",
            manifest_path, out_dir, max_records))

manifest <- fread(manifest_path)

# Statistical primitives copied from extract_extended_features.R (same code path)
...
lomb_top <- function(x, t = NULL, top = 3L) {
  ...
  if (length(x) > 20000L) {
    idx <- sort(sample.int(length(x), 20000L))
    x <- x[idx]; t <- t[ok][idx]
  }
...
spec_top <- function(x, top = 3L) {
  ...
  if (length(x) > 100000L) x <- x[seq.int(1L, length(x), length.out = 100000L)]
...
acf_summary <- function(x, max_lag = 10000L) {
  ...
  data.table(
    ...
    decay_to_5pct_lag = {
      below <- which(abs(vals) < 0.05); if (length(below)) below[1] else NA_real_
    },
...
runs_p <- function(x) {
  ...
  med <- median(x); bin <- factor(ifelse(x > med, "H", "L"))
...
bds_pvalues <- function(x, m = 3L, eps_factor = 0.5) {
  ...
  eps <- eps_factor * sd(x); if (!is.finite(eps) || eps == 0) return(rep(NA_real_, m - 1L))
...
approx_ent <- function(x) {
  ...
  if (length(x) < 50L) return(NA_real_)
  if (length(x) > 2000L) x <- x[seq.int(1L, length(x), length.out = 2000L)]
...
perm_ent <- function(x, m = 4L, tau = 1L) {
  ...
  if (N > 100000L) x <- x[seq.int(1L, N, length.out = 100000L)]
  N <- length(x)
...
lempel_ziv <- function(x, n_bits = 4L, max_n = 50000L) {
  x <- x[is.finite(x)]
  if (!length(x)) return(NA_real_)
  if (length(x) > max_n) x <- x[seq.int(1L, length(x), length.out = max_n)]
  ranks <- as.integer(cut(rank(x, ties.method = "first"), 2^n_bits,
                           labels = FALSE, include.lowest = TRUE))
  N <- length(ranks); if (N < 4L) return(NA_real_)
  dict <- new.env(hash = TRUE, parent = emptyenv())
  c <- 0L; cur <- ""
  for (i in seq_len(N)) {
    cand <- if (cur == "") as.character(ranks[i]) else paste(cur, ranks[i], sep = ",")
    if (exists(cand, envir = dict, inherits = FALSE)) cur <- cand
    else { assign(cand, TRUE, envir = dict); c <- c + 1L; cur <- "" }
  }
  c / (N / log2(N))
}
...
compress_ratio <- function(x) {
  ...
  z <- memCompress(raw, type = "xz"); length(z) / length(raw)
}
...
chisq_uniform_keys <- function(keys, bins = 64L) {
  ...
  obs <- tabulate(k, nbins = bins); exp <- rep(sum(obs) / bins, bins)
  pchisq(sum((obs - exp)^2 / exp), df = bins - 1L, lower.tail = FALSE)
}
...
analyze_series <- function(x, t = NULL, full = TRUE, time_budget_s = 1800) {
  start_t <- Sys.time()
  budget_left <- function() as.numeric(Sys.time() - start_t, units = "secs") < time_budget_s
  out <- list(n = length(x))
  m <- extended_moments_and_cumulants(x); if (!is.null(m)) out <- c(out, m)
  ...
  if (!is.null(gpd)) { out$gpd_xi <- gpd$xi; out$gpd_beta <- gpd$beta; out$gpd_threshold <- gpd$threshold }
  if (!full) return(out)
  ...
  if (!is.null(sp)) { out$spec_period_1 <- sp$period[1]; out$spec_power_1 <- sp$spec[1] }
  ...
  out$change_points <- NA_integer_
  out$hmm_best_states <- NA_integer_
  out
}
...
process_one <- function(row, max_records, dump_py) {
  ...
  if (is.null(records)) return(list(error = "dump failed", path = row$path))
  out <- list(path = row$path, dataset = row$dataset, family = row$family,
  ...
  if ("ts" %in% cols && row$kind == "request_sequence") {
    ts <- records$ts; iat <- diff(ts); iat <- iat[is.finite(iat) & iat >= 0]
    ...
    if ("obj_id" %in% cols) {
      ...
      strides <- diff(key); strides <- strides[is.finite(strides)]
    ...
    if (length(ts) > 100L) {
      bin <- floor(ts - min(ts)); rate <- as.numeric(tabulate(bin + 1L))
      out <- c(out, prefix(analyze_series(rate, full = TRUE), "rate"))
      stl_year <- stl_var_share(rate, period = max(8L, min(86400L, length(rate) %/% 4)))
      out$rate_stl_seasonal_share <- stl_year[1]; out$rate_stl_trend_share <- stl_year[2]
      out$rate_stl_remainder_share <- stl_year[3]
    }
  } else if (row$kind == "aggregate_time_series") {
    metrics <- intersect(c("read_iops", "read_bw", "write_iops", "write_bw", "disk_usage"), cols)
    for (m in metrics) {
      v <- records[[m]]; v <- v[is.finite(v)]
      out <- c(out, prefix(analyze_series(v, full = (m == "read_iops")), m))
    }
  } else {
...
for (i in seq_len(nrow(manifest))) {
  ...
  per_path <- file.path(per_trace_dir,
                        paste0(gsub("/", "_", r$logical_family_id), "__",
                               tools::file_path_sans_ext(basename(r$path)), ".json"))
  writeLines(jsonlite::toJSON(res, auto_unbox = TRUE, pretty = FALSE,
                              null = "null", na = "null", digits = 10), per_path)
  cat(sprintf("  -> %s\n", per_path))
}
...
cat(sprintf("[big] wrote %d feature rows\n", nrow(flat_dt)))
