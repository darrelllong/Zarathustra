#!/usr/bin/env bash
# Run extract_extended_features.R in N parallel processes on the local host.
# Each process gets a distinct chunk of the manifest. Outputs are merged
# at the end into <APPEND_DIR>/{new_features.csv, per_trace/}.
#
# Usage: run_parallel_extract.sh <APPEND_DIR> [N_WORKERS] [MAX_RECORDS]
set -euo pipefail
APPEND=${1:?usage: run_parallel_extract.sh APPEND_DIR [N_WORKERS] [MAX_RECORDS]}
WORKERS=${2:-8}
MAX=${3:-5000000}
...
SCRIPT="$SCRIPT_DIR/extract_extended_features.R"
[[ -f $SCRIPT ]] || { echo "missing $SCRIPT"; exit 2; }

WORK="$APPEND/_workers"
mkdir -p "$WORK"
...
for i in $(seq 0 $((WORKERS-1))); do
  CHUNK="$WORK/chunk.$i.csv"
  WD="$WORK/out.$i"
  mkdir -p "$WD/per_trace"
  Rscript "$SCRIPT" "$CHUNK" "$WD" "$MAX" > "$WORK/log.$i" 2>&1 &
  PIDS+=($!)
done
echo "spawned ${#PIDS[@]} workers"

FAIL=0
for p in "${PIDS[@]}"; do
  if ! wait "$p"; then FAIL=$((FAIL+1)); fi
done
echo "workers done; failures=$FAIL"
